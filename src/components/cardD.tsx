...
export default defineComponent({
  ...
  setup() {
    ...
    watchEffect(() => {
      ...
      ;(offsetRef as unknown as Ref<HTMLDivElement>).value?.style.setProperty(
        '--percent',
        `${state.offset / max.value.offset}`
      )
      ;(rangeRef1 as unknown as Ref<HTMLDivElement>).value?.style.setProperty(
        '--percent',
        `${state.size / state.radius / 2}`
      )
      ;(sizeRef as unknown as Ref<HTMLDivElement>).value?.style.setProperty(
        '--percent',
