...
export default defineComponent({
  ...
  setup() {
    ...
    const style = computed(() => {
      ...
      const split
        ...
        && `, linear-gradient(${
          ...
          || ''
      const size
        = state.direction === 'horizontal' ? `${state.split === 'dashed' ? state.gap * 2 : ~~state.size + ~~state.gap}px ${
              state.size
            }px` : `${state.size}px ${
      ...
      return {
        ...
        '-webkit-mask-position': `${position}, ${
          ...
          state.position === 'end' ? `calc(100% - ${state.offset - state.size / 2}px)` : state.position === 'center' ? '50%' : `${state.offset - state.size / 2}px`
        }`,
    ...
    watchEffect(() => {
      ...
      rangeRef1.value?.style.setProperty(
        '--percent',
        `${state.size / state.radius / 2}`,
      )
      sizeRef.value?.style.setProperty(
        '--percent',
        `${state.gap / state.radius / 2}`,
      )
    })
